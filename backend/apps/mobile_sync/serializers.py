...
class PushItemSerializer(serializers.Serializer):
    ...
    serializer (Invoice, Trip, TripCheckpoint, ...) validates/creates
    `payload`. See mobile_sync.views.PUSH_HANDLERS for the supported set —
    Receipt/CreditNote/SalesOrder follow the identical pattern and are the
    natural next extension of this same list."""

    entity_type = serializers.ChoiceField(choices=["invoice", "trip", "trip_checkpoint"])
