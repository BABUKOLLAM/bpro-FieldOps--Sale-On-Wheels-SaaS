      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-6">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {OFFLINE_STEPS.map((step, i) => (
            <div key={step.title} className="relative rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6">
              <span className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-500 text-sm font-semibold text-neutral-950">
                {i + 1}
              </span>
              <h3 className="mt-4 text-base font-semibold text-neutral-900 dark:text-white">{step.title}</h3>
              <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-300">{step.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-10 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-6 sm:p-8">
          <div className="grid items-center gap-6 sm:grid-cols-[1fr_auto_1fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                Tally Prime → Field App
              </p>
              <p className="mt-1 text-sm font-medium text-neutral-900 dark:text-white">Masters, pulled in</p>
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-neutral-900 dark:text-white">Transactions, pushed out</p>
              <ul className="mt-3 space-y-1.5">
