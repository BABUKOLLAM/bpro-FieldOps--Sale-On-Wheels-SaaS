...
/**
 * Proxies the one-time "set your password" link (uid/token, emailed on
 * signup-request approval — see backend apps.accounts.views.approve) to
 * Django's unauthenticated POST /api/auth/set-password/. No cookie/token
 * involved, same as session/signup-request, since the person hasn't
 * logged in yet — this is what makes their account usable for the first
 * time.
 */
export async function POST(request: NextRequest) {
  const body = await request.json();

  const backendResponse = await fetch(`${API_BASE_URL_INTERNAL}/api/auth/set-password/`, {
    method: "POST",
